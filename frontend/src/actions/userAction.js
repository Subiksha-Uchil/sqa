...
export const loadUser = () => async (dispatch) => {
	try {
		...
		dispatch({ type: LOAD_USER_SUCCESS, payload: data.users });
	} catch (error) {
...
export const updateAccount = (userData) => async (dispatch) => {
	try {
		...
		dispatch({ type: UPDATE_ACCOUNT_SUCCESS, payload: data.success });
	} catch (error) {
		dispatch({
...
export const newMessage = (messageData) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(`/api/v1/message`, messageData, config);

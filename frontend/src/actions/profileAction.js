...
export const getProfile = (keyword=" ", currentPage = 1,salary=[0,25000]) => async (dispatch) => {
    try {
        dispatch({ type: ALL_PROFILE_REQUEST });
        const { data } = await axios.get(`/api/v1/profiles?keyword=${keyword}&page=${currentPage}&salary[gte]=${salary[0]}&salary[lte]=${salary[1]}`);
        dispatch({
